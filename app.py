...
def preprocessed_text(text):
    ...
    padded_review = sequnece.pad_sequences([encoded_review], maxlen =500)
...
if st.button("Classify"):
    if not user_input.strip():
        ...
        if st.checkbox("Show processed review"):
            padded = preprocessed_text(user_input)
            st.write("Encoded Review:", padded.tolist())
            st.write("Decoded Review:", decoded_review(padded[0]))